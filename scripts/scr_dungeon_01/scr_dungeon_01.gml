...
function scr_increment_quest_status(_quest_name)
{
	if (global.quest_status[? _quest_name] == undefined)
	{
		return;
	}
	
	global.quest_status[? _quest_name] += 1;
}

function scr_set_quest_status(_quest_name, _value)
{
	if (global.quest_status[? _quest_name] == undefined)
	{
		return;
	...
	global.quest_status[? _quest_name] = _value;
}